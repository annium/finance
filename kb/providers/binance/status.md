...
| 4 — provider, read paths (+ registration, config, read-only live validation) | **converged** | every read path on both venues driven offline, failure paths included; endpoints pinned; **live read block green: 20 tests, 14 passed, 6 skipped, none failed** | none. One upstream defect found and recorded rather than fixed: an exchange error is discarded when the success type is a collection |
