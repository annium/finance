...
| 4 — provider, read paths (+ registration, config, read-only live validation) | **converged** | read paths driven offline on both venues — exchange info, candles, account context, order and trade history paging; endpoints pinned; **live read block green: 20 tests, 14 passed, 6 skipped, none failed** | none |
...
| 2026-09-02 | 4 — provider | *(no report; the work is in the branch)* | read paths driven offline on both venues; **first live read run**, which failed on spot's server time path — `v1` where the exchange documents `v3`, an oddity the manifest had marked and never checked. Fixed and pinned; the block is green |
